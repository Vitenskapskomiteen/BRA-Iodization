...
So how can we take this into account, and how does sex really matter? If we only include sex into the model, there are in fact three ways in which it could enter:
...
model2c <- MCMCglmm(log(iodine_woS)~sex,random=~idh(at.level(sex,1)):ind+idh(at.level(sex,2)):ind,
                    rcov=~idh(at.level(sex,1)):units+idh(at.level(sex,2)):units,data=dataNK3)
```
